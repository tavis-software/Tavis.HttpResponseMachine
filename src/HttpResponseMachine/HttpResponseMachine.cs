...
namespace Tavis.HttpResponseMachine
{
    ...
    public class HttpResponseMachine<T> : IResponseHandler
    {
        ...
        public delegate Task ResponseAction<T>(T clientstate, IRequestFactory requestFactory, HttpResponseMessage response);
        ...
        public async Task<HttpResponseMessage> HandleResponseAsync(IRequestFactory requestFactory, HttpResponseMessage response)
        {
            var actionKey = ActionKey.CreateActionKey(response, requestFactory?.LinkRelation);

            var selectedAction = FindBestMatchAction(actionKey);

            await selectedAction.ResponseAction(_Model, requestFactory, response);
        ...
        public interface IActionRegistrationBuilder
        {
            ...
            void Then(Func<IRequestFactory, HttpResponseMessage, Task> action);
            void Then<TARGET>(Action<T, IRequestFactory, TARGET> action);
        ...
        private class ActionRegistrationBuilder : IActionRegistrationBuilder  
        {
            ...
            public void Then(Func<IRequestFactory, HttpResponseMessage, Task> action)
            {
                    var reg = new ActionRegistration(_ActionKey, (m, l, r) => action(l, r));
                    _Machine._ResponseActions.Add(reg);
            }

            public void Then<Target>(Action<T, IRequestFactory, Target> responseAction)
            {
