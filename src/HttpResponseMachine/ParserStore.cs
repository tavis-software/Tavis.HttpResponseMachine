...
namespace Tavis.HttpResponseMachine
{
    public class ParserStore
    {
       ...
       public void AddLinkRelationParser<Source, Target>(string linkrelation, Func<Source, Target> translator) where Target : class
       {
        ...
        public Func<HttpContent, Task<Target>> GetMediaTypeParser<Target>(string mediaType)
        {
            var parsers = _MediaTypeParsers.Where(pi => pi.TargetType == typeof(Target) && pi.MediaType == mediaType);
            if (parsers.Count() == 0) return null;
            var parser = parsers.First().Parse;
            return async (c) => (Target)(await parser(c));
        }
